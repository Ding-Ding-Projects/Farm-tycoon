...
export function clampCamera(viewportW = 1280, viewportH = 800, bounds = worldBounds()) {
  ...
  // When `bounds` fits entirely inside the visible span, the feasible range of camera positions
  // is [bounds.max - halfSouth, bounds.min + halfNorth]; its midpoint is the plain bounds-centre
  // shifted NORTH by (halfNorth - halfSouth) / 2, which is what keeps content north of centre
  // (the starting fields) from hiding under the HUD.
